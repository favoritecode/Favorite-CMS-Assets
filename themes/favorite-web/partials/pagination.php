...
$params = $_GET;
unset($params['page']);
$basePathUrl = fw_url(function_exists('site_request_path') ? site_request_path() : '/');
...
sort($pages);
?>
<nav class="pagination" aria-label="<?php echo fw_e($label ?? 'Pagination'); ?>">
    <p class="pagination__status">Page <?php echo $current; ?> of <?php echo $total; ?></p>
    <ul class="pagination__list">
        <li class="pagination__item pagination__item--prev">
            <?php if ($current > 1): ?>
                <a class="pagination__link" href="<?php echo fw_e($pageUrl(min($current - 1, $total))); ?>"><span aria-hidden="true">&larr;</span> Previous</a>
            <?php else: ?>
        ...
        <?php foreach ($pages as $page): ?>
            <?php if ($previousPage > 0 && $page - $previousPage === 2): ?>
                <li class="pagination__item pagination__item--number"><a class="pagination__link" href="<?php echo fw_e($pageUrl($previousPage + 1)); ?>" aria-label="Page <?php echo $previousPage + 1; ?>"><?php echo $previousPage + 1; ?></a></li>
            ...
            <li class="pagination__item pagination__item--number">
                ...
                <?php else: ?>
                    <a class="pagination__link" href="<?php echo fw_e($pageUrl($page)); ?>" aria-label="Page <?php echo $page; ?>"><?php echo $page; ?></a>
                <?php endif; ?>
            </li>
        ...
        <li class="pagination__item pagination__item--next">
            <?php if ($current < $total): ?>
                <a class="pagination__link" href="<?php echo fw_e($pageUrl($current + 1)); ?>">Next <span aria-hidden="true">&rarr;</span></a>
            <?php else: ?>
