...
<section class="comments" id="comments" aria-labelledby="comments-title">
    ...
    <?php if ($commentSuccess !== ''): ?>
        <div class="notice notice--success" role="status"><?php echo fw_e($commentSuccess); ?></div>
    <?php endif; ?>

    <?php if ($commentError !== ''): ?>
        <div class="notice notice--error" role="alert"><?php echo fw_e($commentError); ?></div>
    <?php endif; ?>
        ...
        <ol class="comment-list">
            <?php foreach ($comments as $comment): ?>
                ...
                <li class="comment" id="comment-<?php echo (int)$comment->id; ?>">
                    <div class="comment__avatar" aria-hidden="true"><?php echo fw_e(fw_initial($commentAuthor)); ?></div>
                    <div class="comment__main">
                        <p class="comment__meta">
                            <span class="comment__author"><?php echo fw_e($commentAuthor); ?></span>
                            <a class="comment__permalink" href="#comment-<?php echo (int)$comment->id; ?>"><time datetime="<?php echo fw_e(format_date($comment->created_at, 'c')); ?>"><?php echo fw_e(format_date($comment->created_at, 'M j, Y \a\t g:i a')); ?></time></a>
                        </p>
                        <div class="comment__text"><?php echo nl2br(fw_e($comment->content ?? '')); ?></div>
                    </div>
                </li>
            <?php endforeach; ?>
        </ol>
