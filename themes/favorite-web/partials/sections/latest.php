...
$latestCfg = function_exists('fw_get_section_config') ? fw_get_section_config('latest') : [];

$headingText = (string)($latestCfg['title'] ?? 'Latest Articles');
$headingVisibility = !isset($latestCfg['show_heading']) || !empty($latestCfg['show_heading']);
$showHeading = ($showHeading ?? true) && $headingVisibility;
...
<section class="home-section home-latest"<?php echo $showHeading ? ' aria-labelledby="home-latest-title"' : ' aria-label="Latest articles"'; ?>>
    <?php if ($showHeading && $posts !== []): ?>
        <div class="section-heading">
            <h2 class="section-heading__title" id="home-latest-title"><?php echo fw_e($headingText); ?></h2>
        </div>
    <?php endif; ?>
