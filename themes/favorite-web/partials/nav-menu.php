...
$renderMenu = static function (array $items, int $depth) use (&$renderMenu, $menuClass): void {
    ?>
    <ul class="<?php echo $depth === 0 ? fw_e($menuClass) : 'sub-menu'; ?>">
        <?php foreach ($items as $item): ?>
            ...
            $targetAttr = in_array($target, ['_blank', '_parent', '_top'], true)
                ...
                : '';
            ?>
            <li class="<?php echo fw_e(implode(' ', $classes)); ?>">
                <a class="menu-link" href="<?php echo fw_e($url); ?>"<?php echo $isCurrent ? ' aria-current="page"' : ''; ?><?php echo $targetAttr; ?>><?php echo fw_e($item->title ?? ''); ?></a>
