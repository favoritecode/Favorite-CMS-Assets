...
<section class="empty-state">
    <?php if (!empty($code)): ?>
        <p class="empty-state__code" aria-hidden="true"><?php echo fw_e($code); ?></p>
    <?php endif; ?>
    <<?php echo $headingTag; ?> class="empty-state__title"><?php echo fw_e($title ?? ''); ?></<?php echo $headingTag; ?>>
    <?php if (!empty($message)): ?>
        <p class="empty-state__message"><?php echo fw_e($message); ?></p>
    <?php endif; ?>
    <?php if (!empty($search)) { fw_partial('search-form', $search); } ?>
    <?php if ($actions !== []): ?>
        <div class="empty-state__actions">
            <?php foreach ($actions as $action): ?>
                <a class="button button--<?php echo ($action['variant'] ?? 'primary') === 'secondary' ? 'secondary' : 'primary'; ?>" href="<?php echo fw_e(fw_url((string)($action['url'] ?? '/'))); ?>"><?php echo fw_e($action['label'] ?? ''); ?></a>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
</section>
