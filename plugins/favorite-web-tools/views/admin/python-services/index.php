...
<div class="fwt-admin-wrap" style="max-width: 1100px; margin: 0 auto; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px; margin-bottom: 20px;">
        ...
        <div>
            <a href="/admin/page/favorite-web-tools-python-services?action=create" style="display: inline-flex; align-items: center; gap: 6px; background: #2563eb; color: #fff; text-decoration: none; padding: 8px 16px; border-radius: 6px; font-weight: 600; font-size: 13px;">
                + Add Python Service
            </a>
        </div>
    </div>
                ...
                <?php if (empty($services)): ?>
                    <tr>
                        <td colspan="7" style="padding: 40px; text-align: center; color: #64748b;">
                            No Python API microservices configured yet.
                        </td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr style="border-bottom: 1px solid #f1f5f9;">
                            ...
                            <td style="padding: 12px 16px;">
                                <?php if (!empty($srv->api_key)): ?>
                                    <span style="color: #16a34a; font-size: 12px;">●●● Configured</span>
                                <?php else: ?>
                            ...
                            <td style="padding: 12px 16px; text-align: right; white-space: nowrap;">
                                <a href="/admin/page/favorite-web-tools-python-services?action=edit&id=<?php echo (int)$srv->id; ?>" style="color: #2563eb; text-decoration: none; margin-right: 8px;">Edit</a>

                                <form method="POST" action="/admin/page/favorite-web-tools-python-services" style="display: inline-block; margin-right: 4px;">
                                    <input type="hidden" name="_token" value="<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>">
                                    <input type="hidden" name="action" value="test">
                                    <input type="hidden" name="id" value="<?php echo (int)$srv->id; ?>">
                                    <button type="submit" style="background: none; border: none; color: #0891b2; cursor: pointer; padding: 0; font-size: 13px;">Test</button>
                                </form>

                                <form method="POST" action="/admin/page/favorite-web-tools-python-services" style="display: inline-block;" onsubmit="return confirm('Delete this Python service?');">
                                    <input type="hidden" name="_token" value="<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>">
