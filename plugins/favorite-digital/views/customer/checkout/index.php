...
<head>
    ...
    <style>
        body { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen, Ubuntu, sans-serif; background: var(--bg, #f8fafc); color: var(--text, #1e293b); margin: 0; padding: 24px; }
        .checkout-container { max-width: 800px; margin: 0 auto; background: var(--surface, #ffffff); border: 1px solid var(--border, #e2e8f0); border-radius: 12px; padding: 32px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); }
        h1 { font-size: 24px; margin-top: 0; margin-bottom: 20px; color: var(--heading, #0f172a); border-bottom: 2px solid var(--border, #f1f5f9); padding-bottom: 12px; }
        ...
        .order-summary { background: var(--surface-muted, #f8fafc); border: 1px solid var(--border, #e2e8f0); border-radius: 8px; padding: 20px; margin-bottom: 24px; }
        .summary-row { display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 14px; color: var(--text, #1e293b); }
        .summary-row.total { font-size: 18px; font-weight: 700; color: var(--heading, #0f172a); border-top: 1px solid var(--border-strong, #cbd5e1); padding-top: 12px; margin-top: 12px; }
        .wallet-badge { background: rgba(16, 185, 129, 0.12); border: 1px solid rgba(16, 185, 129, 0.3); color: var(--success, #065f46); padding: 12px 16px; border-radius: 8px; margin-bottom: 24px; display: flex; justify-content: space-between; align-items: center; font-weight: 600; }
        .payment-methods { margin-bottom: 24px; }
        .method-card { border: 2px solid var(--border, #e2e8f0); border-radius: 8px; padding: 16px; margin-bottom: 12px; cursor: pointer; transition: border-color 0.2s; background: var(--surface, #ffffff); color: var(--text, #1e293b); }
        .method-card:hover { border-color: var(--border-strong, #94a3b8); }
        .method-card.selected { border-color: var(--accent, #2563eb); background: var(--accent-soft, #eff6ff); }
        .method-card label { display: flex; align-items: center; gap: 12px; cursor: pointer; font-weight: 600; font-size: 15px; }
        .method-desc { font-size: 13px; color: var(--muted, #64748b); margin-top: 4px; margin-left: 28px; font-weight: normal; }
        .field-group { margin-top: 16px; margin-left: 28px; }
        .field-group label { font-size: 13px; color: var(--muted, #475569); display: block; margin-bottom: 6px; }
        .field-group input, .field-group select { width: 100%; max-width: 320px; padding: 10px 12px; border: 1px solid var(--border-strong, #cbd5e1); border-radius: 6px; font-size: 14px; background: var(--surface, #ffffff); color: var(--text, #1e293b); }
        .btn-pay { width: 100%; padding: 14px; background: var(--accent, #2563eb); color: #ffffff; border: none; border-radius: 8px; font-size: 16px; font-weight: 600; cursor: pointer; transition: background 0.2s; }
        .btn-pay:hover { background: var(--accent-hover, #1d4ed8); }
        .items-list { margin-top: 12px; border-top: 1px dashed var(--border-strong, #cbd5e1); padding-top: 12px; }
        .item-row { display: flex; justify-content: space-between; font-size: 13px; color: var(--muted, #475569); margin-bottom: 4px; }
        .manual-box { background: var(--surface-muted, #f1f5f9); border: 1px solid var(--border-strong, #cbd5e1); border-radius: 8px; padding: 16px; margin-top: 14px; margin-left: 28px; font-size: 13px; }
        .manual-box h3 { margin: 0 0 10px; font-size: 14px; font-weight: 700; color: var(--heading, #0f172a); }
        .manual-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 12px; }
        .manual-item { background: var(--surface, #ffffff); border: 1px solid var(--border, #e2e8f0); border-radius: 6px; padding: 8px 12px; }
        .manual-item-lbl { font-size: 11px; font-weight: 600; color: var(--muted, #64748b); text-transform: uppercase; margin-bottom: 2px; }
        .manual-item-val { font-size: 14px; font-weight: 700; color: var(--heading, #0f172a); }
        .manual-instructions { background: rgba(245, 158, 11, 0.12); border: 1px solid rgba(245, 158, 11, 0.3); border-radius: 6px; padding: 10px 12px; color: #d97706; margin-bottom: 12px; font-size: 13px; }
        ...
        .manual-inputs-grid label { font-size: 12px; font-weight: 600; color: var(--text, #334155); display: block; margin-bottom: 4px; }
        .manual-inputs-grid input, .manual-inputs-grid textarea { width: 100%; box-sizing: border-box; padding: 8px 10px; border: 1px solid var(--border-strong, #cbd5e1); border-radius: 6px; font-size: 13px; background: var(--surface, #ffffff); color: var(--text, #1e293b); }
    </style>
</head>
<body>
