...
<div class="fd-admin-wrap" style="max-width: 1200px; margin: 0 auto; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
    ...
    <!-- Bulk Actions Form -->
    <form id="fd-packages-bulk-form" method="POST" action="/admin/page/favorite-digital-packages">
        <input type="hidden" name="_token" value="<?php echo htmlspecialchars($csrfToken, ENT_QUOTES, 'UTF-8'); ?>">
        <input type="hidden" name="action" value="bulk_action">
        <input type="hidden" name="redirect_to" value="<?php echo htmlspecialchars($_SERVER['REQUEST_URI'] ?? '/admin/page/favorite-digital-packages', ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 13px;">
                <thead>
                    <tr style="background: #f8f9fa; border-bottom: 1px solid #c3c4c7; color: #50575e; font-weight: 600;">
                        <th style="padding: 12px 14px; width: 40px; text-align: center;">
                            <input type="checkbox" data-select-all aria-label="Select all packages on this page">
                        </th>
                ...
                <tbody>
                    <?php if (empty($packages)): ?>
                        <tr>
                            <td colspan="6" style="padding: 32px 14px; text-align: center; color: #646970;">
                                No packages found.
                        ...
                        <?php foreach ($packages as $p): ?>
                            <tr style="border-bottom: 1px solid #f0f0f1;">
                                <td style="padding: 12px 14px; text-align: center;">
                                    <input type="checkbox" name="ids[]" value="<?php echo (int)$p->id; ?>" aria-label="Select package #<?php echo (int)$p->id; ?>">
                                </td>
                                ...
                                <td style="padding: 12px 14px;">
                                    <?php
                                    $pkgCurrency = $p->currency ?? (class_exists(\FavoriteCMS\Core\Currency::class) ? \FavoriteCMS\Core\Currency::getPrimaryCurrency() : 'BDT');
                                    ...
                                    <?php if (!empty($p->is_free)): ?>
                                        <span style="display: inline-block; background: #e7f7ed; color: #155724; font-weight: 600; padding: 2px 6px; border-radius: 3px; font-size: 12px;">FREE (<?= htmlspecialchars($pkgSymbol, ENT_QUOTES, 'UTF-8') ?>0)</span>
                                    <?php else: ?>
                                        <div style="font-weight: 600; color: #1e1e1e;">
                                            <?= htmlspecialchars($pkgSymbol, ENT_QUOTES, 'UTF-8') ?><?php echo htmlspecialchars(number_format((float)$p->final_price, 2), ENT_QUOTES, 'UTF-8'); ?>
                                        </div>
                                        <?php if ((float)$p->discount_percent > 0): ?>
                                            <div style="font-size: 11px; color: #8c8f94; text-decoration: line-through;">
                                                <?= htmlspecialchars($pkgSymbol, ENT_QUOTES, 'UTF-8') ?><?php echo htmlspecialchars(number_format((float)$p->original_price, 2), ENT_QUOTES, 'UTF-8'); ?>
                                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    if (typeof window.initAdminMultiSelect === 'function') {
        window.initAdminMultiSelect('fd-packages-bulk-form', { itemType: 'package' });
    }
});
</script>
