            ...
            <div style="display: flex; flex-direction: column; gap: 20px;">
                ...
                <div style="background: #fff; border: 1px solid #c3c4c7; border-radius: 4px; padding: 20px; box-shadow: 0 1px 1px rgba(0,0,0,0.04);">
                    ...
                    <div id="fd-url-block" style="margin-bottom: 16px; display: <?php echo in_array($currentResType, ['url', 'both'], true) ? 'block' : 'none'; ?>;">
                        ...
                        <div>
                            <label style="display: block; font-weight: 600; font-size: 13px; margin-bottom: 6px; color: #1e1e1e;">
                                External File Size (Manual Input)
                            </label>
                            <?php 
                                $existingBytes = (int)($details->file_size ?? $details->file_size_bytes ?? 0);
                                $manualSizeVal = $old['manual_file_size_val'] ?? '';
                                $manualSizeUnit = $old['manual_file_size_unit'] ?? 'MB';
                                if ($manualSizeVal === '' && $existingBytes > 0) {
                                    if ($existingBytes >= 1073741824) {
                                        $manualSizeVal = round($existingBytes / 1073741824, 2);
                                        $manualSizeUnit = 'GB';
                                    } elseif ($existingBytes >= 1048576) {
                                        $manualSizeVal = round($existingBytes / 1048576, 2);
                                        $manualSizeUnit = 'MB';
                                    } elseif ($existingBytes >= 1024) {
                                        $manualSizeVal = round($existingBytes / 1024, 2);
                                        $manualSizeUnit = 'KB';
                                    } else {
                                        $manualSizeVal = $existingBytes;
                                        $manualSizeUnit = 'B';
                                    }
                                }
                            ?>
                            <div style="display: flex; gap: 8px; align-items: center;">
                                <input type="number" step="any" min="0" name="manual_file_size_val" id="fd-manual-file-size-val" value="<?php echo htmlspecialchars((string)$manualSizeVal, ENT_QUOTES, 'UTF-8'); ?>" placeholder="e.g. 150" style="flex: 1; box-sizing: border-box; padding: 8px 12px; border: 1px solid #8c8f94; border-radius: 4px; font-size: 13px;">
                                <select name="manual_file_size_unit" id="fd-manual-file-size-unit" style="width: 100px; padding: 8px 12px; border: 1px solid #8c8f94; border-radius: 4px; font-size: 13px; background: #fff;">
                                    <option value="MB" <?php echo $manualSizeUnit === 'MB' ? 'selected' : ''; ?>>MB</option>
                                    <option value="GB" <?php echo $manualSizeUnit === 'GB' ? 'selected' : ''; ?>>GB</option>
                                    <option value="KB" <?php echo $manualSizeUnit === 'KB' ? 'selected' : ''; ?>>KB</option>
                                    <option value="B" <?php echo $manualSizeUnit === 'B' ? 'selected' : ''; ?>>Bytes</option>
                                </select>
                            </div>
                            <div style="font-size: 11px; color: #646970; margin-top: 4px;">
                                Manually specify the file size for external sources so customers can see the download size.
                            </div>
                        </div>
                    </div>
                        ...
                        <?php if (!empty($details->file_path)): ?>
                            <div style="background: #f8f9fa; border: 1px solid #e2e3e5; border-radius: 4px; padding: 12px 14px; margin-bottom: 16px;">
                                ...
                                <div style="font-size: 12px; color: #646970; margin-top: 4px;">
                                    Size: <?php echo htmlspecialchars(number_format(((int)($details->file_size ?? $details->file_size_bytes ?? 0)) / 1024, 1), ENT_QUOTES, 'UTF-8'); ?> KB
