...
class FulfillmentService
{
     ...
     */
    public function fulfillOrder(int $orderId, ?string $targetStatus = null): object
    {
        ...
        // 3. Idempotency check: if already fully fulfilled and no explicit status override, return cleanly
        if ($targetStatus === null && $order->fulfillment_status === OrderLifecycleState::FULFILLMENT_FULFILLED) {
            return $order;
        }
        ...
        $hasService = false;
        foreach ($items as $item) {
            if ((string)$item->product_type === ProductType::SERVICE) {
                $hasService = true;
                break;
            }
        }

        // Determine new fulfillment status & order status
        if ($targetStatus !== null && OrderLifecycleState::isValidStatus($targetStatus)) {
            $this->orderRepo->updateOrderStatus($orderId, $targetStatus);
            ...
            } elseif ($targetStatus === OrderLifecycleState::STATUS_PROCESSING) {
        ...
        } elseif ($failedCount === 0 && $fulfilledCount === $totalItems) {
            $this->orderRepo->updateFulfillmentStatus($orderId, OrderLifecycleState::FULFILLMENT_FULFILLED);
