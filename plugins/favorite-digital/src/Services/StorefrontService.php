...
class StorefrontService
{
        ...
        if ($type === ProductType::DIGITAL) {
            ...
            if ($details) {
                ...
                $typeDetails = [
                    ...
                    'mime_type'              => ($details->resource_type ?? 'file') === 'url' && (empty($details->mime_type) || $details->mime_type === 'application/octet-stream')
                                                ? 'External Download'
                                                : (string)($details->mime_type ?? 'application/octet-stream'),
    ...
    public function resolveCustomerState(object $product, ?int $userId = null): array
    {
        ...
        // 2. Authenticated: check if customer already holds an active entitlement or service order
        ...
        if ($hasEntitlement) {
            ...
            if ($productType === ProductType::SERVICE) {
                $latestOrder = $this->orderService->getOrderRepository()->findLatestOrderByUserAndProduct($userId, $productId);
                $serviceStatus = $latestOrder ? (string)$latestOrder->status : 'pending';
                $statusDisplay = match ($serviceStatus) {
                    'processing' => 'Processing',
                    'completed', 'complete' => 'Complete',
                    default => 'Pending',
                };
                $orderNumber = $latestOrder ? (string)$latestOrder->order_number : '';
                return [
                    'state'               => 'owned',
                    'label'               => "Service: {$statusDisplay}",
                    'badge'               => $statusDisplay,
                    'service_status'      => strtolower($statusDisplay),
                    'service_status_label'=> match ($serviceStatus) {
                        'processing' => '⚙️ Processing',
                        'completed', 'complete' => '✅ Complete',
                        default => '⏳ Pending',
                    },
                    'order_number'        => $orderNumber,
                    'button_text'         => 'View Access',
                    'button_class'        => 'btn-success',
                    'action_url'          => $orderNumber !== '' ? "/account/orders/{$orderNumber}" : '/account/orders',
                    'is_purchasable'      => false,
                    'is_owned'            => true,
        ...
        // Check if user has an active/pending service order
        if ($productType === ProductType::SERVICE) {
            $latestOrder = $this->orderService->getOrderRepository()->findLatestOrderByUserAndProduct($userId, $productId);
            if ($latestOrder && in_array($latestOrder->status, ['pending', 'processing'], true)) {
                $statusDisplay = ($latestOrder->status === 'processing') ? 'Processing' : 'Pending';
                return [
                    'state'               => 'owned',
                    'label'               => "Service: {$statusDisplay}",
                    'badge'               => $statusDisplay,
                    'service_status'      => strtolower($statusDisplay),
                    'service_status_label'=> ($latestOrder->status === 'processing') ? '⚙️ Processing' : '⏳ Pending',
                    'order_number'        => (string)$latestOrder->order_number,
                    'button_text'         => 'View Access',
                    'button_class'        => 'btn-success',
                    'action_url'          => "/account/orders/{$latestOrder->order_number}",
                    'is_purchasable'      => false,
                    'is_owned'            => true,
                    'requires_membership' => false,
                ];
