...
class DownloadService
{
    ...
    public function authorizeDownload(string $token, int $authenticatedUserId): array
    {
        ...
        if (!$details || (!$hasFile && !$hasUrl)) {
            throw DownloadException::fileNotFound("No downloadable file or resource attached to this product.");
        }
        ...
        // Check if file or external URL resource is available
        $hasFile = !empty($details->file_path);
        $hasUrl = in_array($details->resource_type ?? 'file', ['url', 'both'], true) && !empty($details->resource_url);

        if (!$hasFile && !$hasUrl) {
            throw DownloadException::fileUnavailable();
        }
        ...
        if ($hasFile) {
            ...
            $downloadFileName = !empty($details->file_name)
                ...
                : (string)($product->title ?? 'External Resource');
        }

        $mimeType = !empty($details->mime_type) ? (string)$details->mime_type : 'application/octet-stream';
        $fileSize = !empty($details->file_size) ? (int)$details->file_size : ($absolutePath ? (int)@filesize($absolutePath) : 0);
