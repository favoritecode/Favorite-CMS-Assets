...
class ProductManagementService
{
    ...
    public function validateDigitalDetails(array $input, ?array $existing = null, bool $isPublishing = false): array
    {
        ...
        // Handle manual file size specification (especially useful for external URLs or overrides)
        $manualVal = $input['manual_file_size_val'] ?? null;
        $manualUnit = (string)($input['manual_file_size_unit'] ?? 'MB');
        $manualBytes = null;
        if ($manualVal !== null && $manualVal !== '' && is_numeric($manualVal)) {
            $num = (float)$manualVal;
            if ($num >= 0) {
                $manualBytes = match (strtoupper(trim($manualUnit))) {
                    'GB' => (int)round($num * 1024 * 1024 * 1024),
                    'KB' => (int)round($num * 1024),
                    'B'  => (int)round($num),
                    default => (int)round($num * 1024 * 1024), // MB default
                };
            }
        } elseif (isset($input['manual_file_size']) && is_string($input['manual_file_size']) && trim($input['manual_file_size']) !== '') {
            $manualBytes = $this->parseSizeToBytes(trim($input['manual_file_size']));
        }

        if ($manualBytes !== null) {
            $fileSize = $manualBytes;
        } else {
    ...
    public function parseSizeToBytes(string $input): int
    {
        $input = trim($input);
        if (preg_match('/^([\d.]+)\s*(gb|mb|kb|b)?$/i', $input, $m)) {
            $val = (float)$m[1];
            $unit = strtolower($m[2] ?? 'b');
            return match ($unit) {
                'gb' => (int)round($val * 1024 * 1024 * 1024),
                'mb' => (int)round($val * 1024 * 1024),
