            ...
            if ($pType === ProductType::DIGITAL) {
                ...
                if ($item['state'] === 'accessible') {
                    if ($hasFileResource) {
                        ...
                        // URL only: generate download token for uniform download flow
                        $tokenRecord = $this->downloadService->getOrCreateDownloadToken(
                            $userId,
                            $pid,
                            $item['primary_entitlement_id'] > 0 ? $item['primary_entitlement_id'] : null
                ...
                $latestOrder = $this->orderRepo->findLatestOrderByUserAndProduct($userId, $pid);
                $rawStatus = $latestOrder ? (string)$latestOrder->status : 'pending';
                $serviceStatus = match ($rawStatus) {
                ...
                $item['service_status_label'] = $statusLabel;
                $item['order_number'] = $orderNumber;
                $item['action_url'] = $orderNumber !== '' ? "/account/orders/{$orderNumber}" : "/store/{$item['slug']}";
                $item['action_label'] = 'View Access';
                if ($item['state'] === 'accessible') {
                    $item['state_label'] = $statusLabel;
                }
