...
.fpay-step-circle.active {
    ...
    color: #ffffff;
    box-shadow: 0 0 0 4px var(--accent-soft, #e0f2fe);
}
...
<div style="max-width: 680px; margin: 0 auto;">
    <div style="margin-bottom: 20px;">
        <a href="/account/withdraw" style="color: var(--accent, #2563eb); text-decoration: none; font-size: 14px; font-weight: 600; display: inline-flex; align-items: center; gap: 6px;">
    ...
    <div class="fpay-card">
        <div class="fpay-card-header">
            <div>
                ...
                <span style="font-family: monospace; font-size: 13px; color: var(--muted, #64748b);"><?php echo htmlspecialchars($withdrawal->getId(), ENT_QUOTES, 'UTF-8'); ?></span>
            </div>
            <div>
        ...
        <!-- Status Timeline -->
        <div style="background: var(--surface-muted, #f8fafc); border: 1px solid var(--border, #e2e8f0); border-radius: 8px; padding: 20px 16px 10px; margin-bottom: 24px;">
            <div style="font-size: 12px; font-weight: 700; text-transform: uppercase; color: var(--muted, #64748b); margin-bottom: 12px; text-align: center;">
                Status Timeline
            </div>
        ...
        <!-- Financial Breakdown -->
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px; background: var(--surface, #ffffff); padding: 18px; border-radius: 10px; border: 1px solid var(--border, #e2e8f0);">
            <div>
                <div style="font-size: 12px; color: var(--muted, #64748b); text-transform: uppercase; font-weight: 600; margin-bottom: 4px;">Requested Amount</div>
                <div style="font-size: 20px; font-weight: 800; color: var(--heading, #0f172a);"><?php echo $curr . ' ' . $amountMajor; ?></div>
            </div>
            <div>
                <div style="font-size: 12px; color: var(--muted, #64748b); text-transform: uppercase; font-weight: 600; margin-bottom: 4px;">Net Payable</div>
                <div style="font-size: 20px; font-weight: 800; color: var(--success, #15803d);"><?php echo $curr . ' ' . $netMajor; ?></div>
            </div>
            <div>
                <div style="font-size: 12px; color: var(--muted, #64748b); text-transform: uppercase; font-weight: 600; margin-bottom: 4px;">Processing Fee</div>
                <div style="font-size: 15px; font-weight: 600; color: var(--text, #475569);"><?php echo $curr . ' ' . $feeMajor; ?></div>
            </div>
            <div>
                <div style="font-size: 12px; color: var(--muted, #64748b); text-transform: uppercase; font-weight: 600; margin-bottom: 4px;">Payout Method</div>
                <div style="font-size: 15px; font-weight: 600; color: var(--heading, #0f172a); text-transform: capitalize;"><?php echo htmlspecialchars(str_replace('_', ' ', $withdrawal->getMethod()), ENT_QUOTES, 'UTF-8'); ?></div>
            </div>
        </div>

        <!-- Destination Details -->
        <div style="border-top: 1px solid var(--border, #f1f5f9); padding-top: 18px; margin-bottom: 20px;">
            <h3 style="font-size: 15px; font-weight: 700; color: var(--heading, #1e293b); margin: 0 0 12px;">Destination Details</h3>
            <div style="font-size: 14px; color: var(--text, #334155); line-height: 1.6;">
                <div>Masked Account: <strong style="font-family: monospace; color: var(--heading, #0f172a);"><?php echo htmlspecialchars($withdrawal->getDestinationMasked(), ENT_QUOTES, 'UTF-8'); ?></strong></div>
                <?php if ($withdrawal->getTransactionReference()): ?>
                    <div style="margin-top: 6px;">Transaction Reference / TrxID: <strong style="font-family: monospace; color: var(--accent, #2563eb);"><?php echo htmlspecialchars($withdrawal->getTransactionReference(), ENT_QUOTES, 'UTF-8'); ?></strong></div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <?php if (!empty($notifications)): ?>
            <div style="margin-bottom: 24px;">
                <div style="font-size: 13px; font-weight: 700; text-transform: uppercase; color: var(--muted, #64748b); margin-bottom: 12px;">
                    Activity &amp; Updates
                </div>
                <div style="display: flex; flex-direction: column; gap: 8px;">
                    <?php foreach ($notifications as $notif): ?>
                        <div style="padding: 12px 14px; background: var(--surface-muted, #f8fafc); border: 1px solid var(--border, #e2e8f0); border-radius: 6px; display: flex; justify-content: space-between; align-items: center; gap: 12px;">
                            <div>
                                <div style="font-size: 14px; font-weight: 600; color: var(--heading, #0f172a);">
                                    <?php echo htmlspecialchars((string)($notif['title'] ?? 'Update'), ENT_QUOTES, 'UTF-8'); ?>
                                </div>
                                <div style="font-size: 13px; color: var(--text, #475569); margin-top: 2px;">
        ...
        <!-- Actions -->
        <div style="display: flex; gap: 12px; justify-content: space-between; align-items: center; border-top: 1px solid var(--border, #f1f5f9); padding-top: 20px;">
            ...
            <?php if ($canCancel): ?>
                <div>
                    <form method="POST" action="/account/withdrawals/<?php echo urlencode($withdrawal->getId()); ?>" onsubmit="return confirm('Are you sure you want to cancel this withdrawal request? Held funds will be released back to your available balance.');">
                        ...
                        <button type="submit" class="fpay-btn" style="background: var(--danger-soft, #fee2e2); color: var(--danger, #991b1b); border: 1px solid var(--danger-border, #fca5a5); font-size: 13px;">
                            Cancel Request
                        </button>
                    </form>
                </div>
