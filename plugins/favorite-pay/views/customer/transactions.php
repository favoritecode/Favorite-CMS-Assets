...
<div class="fpay-card" style="margin-bottom: 24px;">
    <div class="fpay-card-header" style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px;">
        <div>
            <h2 class="fpay-card-title" style="margin: 0; font-size: 20px; font-weight: 700; color: var(--heading, #0f172a);">Wallet Transaction Ledger</h2>
        ...
        <div style="text-align: right;">
            <span style="font-size: 12px; color: var(--muted, #64748b); display: block;">Spendable Balance:</span>
            <strong style="font-size: 22px; color: var(--success, #15803d);">
    ...
    <!-- Filter Form -->
    <form method="GET" action="/account/transactions" style="background: var(--surface-muted, #f8fafc); border: 1px solid var(--border, #e2e8f0); border-radius: 8px; padding: 16px; margin: 16px 0 24px;">
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 12px; align-items: flex-end;">
            <div>
                <label style="display: block; font-size: 12px; font-weight: 600; color: var(--muted, #64748b); margin-bottom: 4px;">Type</label>
                <select name="type" style="width: 100%; padding: 8px 10px; font-size: 13px; border: 1px solid var(--border-strong, #cbd5e1); border-radius: 6px; background: var(--surface, #fff); color: var(--text, #1e293b);">
            ...
            <div>
                <label style="display: block; font-size: 12px; font-weight: 600; color: var(--muted, #64748b); margin-bottom: 4px;">Direction</label>
                <select name="direction" style="width: 100%; padding: 8px 10px; font-size: 13px; border: 1px solid var(--border-strong, #cbd5e1); border-radius: 6px; background: var(--surface, #fff); color: var(--text, #1e293b);">
            ...
            <div style="grid-column: span 2;">
                <label style="display: block; font-size: 12px; font-weight: 600; color: var(--muted, #64748b); margin-bottom: 4px;">Search</label>
                <input type="text" name="search" placeholder="Search reference, description, or entry ID..." value="<?php echo htmlspecialchars($filters['search'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" style="width: 100%; padding: 7px 10px; font-size: 13px; border: 1px solid var(--border-strong, #cbd5e1); border-radius: 6px; background: var(--surface, #fff); color: var(--text, #1e293b);">
            </div>
        </div>
    ...
    <?php if (empty($entries)): ?>
        <div style="text-align: center; padding: 48px 20px; color: var(--muted, #64748b); background: var(--surface, #fff); border-radius: 8px; border: 1px dashed var(--border-strong, #cbd5e1);">
            ...
            <p style="margin: 0; font-size: 15px; font-weight: 600; color: var(--heading, #0f172a);">No ledger entries found matching your criteria.</p>
            <?php if (!empty($queryParams)): ?>
                <p style="margin: 6px 0 0; font-size: 13px;"><a href="/account/transactions" style="color: var(--accent, #2563eb); text-decoration: none;">Clear all filters</a></p>
            <?php endif; ?>
        </div>
    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($refType === 'payment' && !empty($refId)): ?>
                                    <a href="/account/payments/<?php echo urlencode($refId); ?>" style="color: var(--accent, #2563eb); text-decoration: none; font-weight: 600; font-family: monospace; font-size: 12px;">
                                        <?php echo htmlspecialchars($refId, ENT_QUOTES, 'UTF-8'); ?>
                                    </a>
                                <?php elseif ($refType === 'withdrawal' && !empty($refId)): ?>
                                    <a href="/account/withdrawals/<?php echo urlencode($refId); ?>" style="color: var(--accent, #2563eb); text-decoration: none; font-weight: 600; font-family: monospace; font-size: 12px;">
                                        <?php echo htmlspecialchars($refId, ENT_QUOTES, 'UTF-8'); ?>
                                    </a>
                                <?php elseif (!empty($refId)): ?>
                                    <span style="font-family: monospace; font-size: 12px; color: var(--muted, #64748b);"><?php echo htmlspecialchars($refId, ENT_QUOTES, 'UTF-8'); ?></span>
                                <?php else: ?>
                                    <span style="color: var(--muted, #94a3b8);">&mdash;</span>
                                <?php endif; ?>
                            </td>
                            <td>
                            ...
                            <td>
                                <span style="font-size: 12px; font-weight: 600; color: <?php echo $isCredit ? 'var(--success, #15803d)' : 'var(--danger, #b91c1c)'; ?>;">
                                    <?php echo $isCredit ? 'Credit (In)' : 'Debit (Out)'; ?>
                                </span>
                            </td>
                            <td style="font-size: 13px; color: var(--text, #1e293b); max-width: 260px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                <?php echo htmlspecialchars($entry->getDescription(), ENT_QUOTES, 'UTF-8'); ?>
                            </td>
                            <td style="text-align: right; font-weight: 700; font-size: 13px; color: <?php echo $isCredit ? 'var(--success, #15803d)' : 'var(--danger, #b91c1c)'; ?>; white-space: nowrap;">
